...
class _SwapDrawerState extends ConsumerState<SwapDrawer> {
  ...
  Widget build(BuildContext context) {
    ...
    final ings = ref.watch(allIngredientsProvider).asData?.value ?? const <dynamic>[];
    ...
    if (widget.isLoading) {
      ...
      subsAsync.whenOrNull(data: (scores) {
        ...
        for (final s in scores) {
          ...
          // Allergen/diet guardrails
          if (strict && (reqDiet.isNotEmpty || pickedAllergens.isNotEmpty)) {
            final violates = AllergenClassifier.violatesDiet(r, reqDiet);
            bool allergen = false;
            if (!violates && pickedAllergens.isNotEmpty && ingById.isNotEmpty) {
              final set = AllergenClassifier.allergensForRecipe(r, ingById);
              allergen = set.any((a) => pickedAllergens.contains(a));
            }
            if (violates || allergen) continue; // hard filter in strict mode
          }
          ...
          if (!strict && (reqDiet.isNotEmpty || pickedAllergens.isNotEmpty)) {
            ...
            if (violates || allergen) local -= 2.0; // down-rank
          }
      ...
      content = smartRows.isNotEmpty
          ? ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                if (!violatesDiet && pickedAllergens.isNotEmpty && ingById.isNotEmpty) {
                  final set = AllergenClassifier.allergensForRecipe(r, ingById);
                            ...
                            if (!strict && (violatesDiet || allergenConflict))
                              Container(
                                ...
                                child: Row(
                                  mainAxisSize: MainAxisSize.min,
                                  children: [
                                    Icon(Icons.warning_amber_outlined, size: 12, color: Theme.of(context).colorScheme.onErrorContainer),
                                    const SizedBox(width: 4),
                                    Text(
                                      violatesDiet ? 'Diet mismatch' : 'Allergen',
